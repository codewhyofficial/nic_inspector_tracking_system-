<nav class="flex justify-between bg-[#0a1333] py-3 sticky top-0 w-full z-10">
    <div class="flex items-center ml-9 gap-5">
        @if(Session::get('role') == 'admin') <span onclick="window.location='/admin'" class="text-xl text-white font-semibold cursor-pointer border-r-2 border-white pr-4">Admin</span> @endif
        <span class="text-md text-gray-200 cursor-pointer transition duration-300 ease-in-out mr-6 hover:bg-[#0d1c4e] p-2 rounded-md">{{ $user->name }}</span>
    </div>
