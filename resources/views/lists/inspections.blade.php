...
<div class="container mx-auto mt-5 flex flex-col justify-center">
    <div class="flex justify-between items-center mb-2">
        ...
        <div>
            <button onclick="window.location='{{ route('addInspection', ['uiid' => $user->uiid]) }}'" class=" bg-blue-600 hover:bg-blue-700 p-2 rounded-md text-white"><i class="fas fa-plus mr-2"></i>Add</button>
        </div>
    </div>
    <div>
        @if(empty($inspections))
        <p class="text-gray-700 text-center">No inspection details available.</p>
        @else
        <table class="min-w-full bg-white">
            <thead class="bg-blue-950 text-white">
                <tr>
                    <th class="text-left py-2 px-4 uppercase font-semibold text-sm">S.no.</th>
                    <th class="text-left py-2 px-4 uppercase font-semibold text-sm">Name</th>
                    <th class="text-left py-2 px-4 uppercase font-semibold text-sm">Inspection Category</th>
                    <th class="text-left py-2 px-4 uppercase font-semibold text-sm">Date of Joining</th>
            ...
            <tbody id="inspectorTableBody" class="text-gray-700">
                ...
                <tr class="border border-gray-200 hover:bg-gray-100 hover:rounded-md cursor-pointer hover-scale bg-white">
                    ...
                    <td class="text-left py-2 px-4">{{ $inspection->name }}</td>
                    <td class="text-left py-2 px-4">{{ $inspection->inspection_category }}</td>
